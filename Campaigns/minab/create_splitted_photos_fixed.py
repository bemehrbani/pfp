#!/usr/bin/env python3
"""Create final splitted_photos folder with best available names - FIXED VERSION."""
...
def is_valid_name_strict(text):
    ...
    # Simplified version - just check basic validity
    if not text:
        return False
    ...
    # Check for excessive numbers or weird characters
    digit_count = sum(c.isdigit() for c in text)
    if digit_count > 1:
        return False

    # Check for obviously non-name characters
    non_name_chars = '=|\\/:;°¬~`@#$%^&*()_+[]{}<>?!"\''
...
def main():
    ...
    # Create splitted_photos folder (clean if exists)
    ...
    # Verify final count
    final_files = len([f for f in os.listdir(dest_dir) if f.lower().endswith('.jpg')])
    print(f"Files in {dest_dir}: {final_files} (should be 100)")
