#!/usr/bin/env python3
"""
Final Video V2 — Polished grid video + Suno vocals + beat mix.
...
# Duration matches the Suno track
TOTAL_DURATION = 197.2
TOTAL_FRAMES = int(TOTAL_DURATION * FPS)

PHOTO_DIR = Path(__file__).parent / "splitted_photos"
VOCAL_FILE = Path(__file__).parent / "music" / "100 Faces of Peace — Rap Lyrics.mp3"
...
# --- Lyrics sync (corrected from RMS audio analysis of Suno track) ---
# Structure: intro(0-11), buildup(11-22), verse1(22-44), hook1(45-67),
#            verse2(68-88), hook2(90-112), bridge(112-135), final(135-180), fade(180+)
LYRICS_TIMED = [
    # Intro (0-11s — quiet, instrumental only)
    (0.0, ""),
    # Buildup (11-22s — vocals start fading in)
    (11.0, "Look at their eyes, look at their face"),
    (14.5, "A hundred children from a forgotten place"),
    (17.5, "Minab to the world, we carry their voice"),
    (20.0, "Peace ain't a privilege — it's a right, not a choice"),
    # Verse 1 (22-44s — LOUD section)
    (22.0, ""),
    (23.0, "Zahra wants a pencil, Parsa wants a book"),
    (25.8, "Amirali dreams of a future — take a look"),
    (28.5, "Asma paints in colors that the world won't see"),
    (31.0, "Sobhan's in a classroom where he can't be free"),
    (34.0, "A hundred names, a hundred faces on the wall"),
    (36.5, "Every single one of them too young to fall"),
    (39.0, "Reza, Makan, Sina — do you hear them call?"),
    (41.5, "From Minab to the world, we speak for them all"),
    # Hook 1 (45-67s)
    (44.0, ""),
    (45.5, "🔊 Dub dub dub — DISH — can you hear the beat?"),
    (48.5, "A hundred hearts are marching down the street"),
    (51.0, "🔊 Dub dub dub — DISH — every face you see"),
    (54.0, "Is a child who's asking: Won't you fight for me?"),
    (56.5, "🔊 Dub dub dub — DISH — can you hear the beat?"),
    (59.5, "A hundred hearts are marching down the street"),
    (62.0, "🔊 Dub dub dub — DISH — every face you see"),
    (65.0, "Is a child who's asking: Won't you fight for me?"),
    # Verse 2 (68-88s)
    (67.5, ""),
    (68.5, "Sanctions hit the hospitals, the schools, the bread"),
    (71.5, "Politicians sleeping while the children aren't fed"),
    (74.5, "She was gonna be a doctor, he was gonna fly"),
    (77.5, "But the world looked away and nobody asked why"),
    (80.0, "Every face a statistic, every name erased"),
    (83.0, "Every dream is broken, every hope displaced"),
    (85.5, "But we don't scroll past — nah, we stop and stare"),
    (87.5, "'Cause a hundred children need to know we care"),
    # Hook 2 (90-112s)
    (89.0, ""),
    (90.5, "🔊 Dub dub dub — DISH — can you hear the beat?"),
    (93.5, "A hundred hearts are marching down the street"),
    (96.0, "🔊 Dub dub dub — DISH — every face you see"),
    (99.0, "Is a child who's asking: Won't you fight for me?"),
    (101.5, "🔊 Dub dub dub — DISH — can you hear the beat?"),
    (104.5, "A hundred hearts are marching down the street"),
    (107.0, "🔊 Dub dub dub — DISH — every face you see"),
    (109.5, "Is a child who's asking: Won't you fight for me?"),
    # Bridge (112-135s — quieter section)
    (112.0, ""),
    (115.0, "One share is a ripple, a hundred is a wave"),
    (121.0, "One voice in the silence might be all it takes to save"),
    (127.0, "Look 'em in the eyes — they're watching you"),
    (132.0, "What are you gonna do?"),
    # Final Section (135-180s — big finish)
    (135.0, ""),
    (137.0, "A hundred faces... a hundred dreams..."),
    (143.0, "People for Peace — that's what it means"),
    (149.0, "Minab to the world, we won't be ignored"),
    (155.0, "Every. Child. Deserves. More."),
    (161.0, ""),
    (163.0, "🔊 Dub dub dub — DISH — can you hear the beat?"),
    (166.0, "A hundred hearts are marching down the street"),
    (169.0, "🔊 Dub dub dub — DISH — every face you see"),
    (172.0, "Is a child who's asking: Won't you fight for me?"),
    (175.0, "Every. Child. Deserves. More."),
    # Fade out (180+)
    (180.0, ""),
    (183.0, "#100FacesOfPeace  ·  People for Peace"),
    (192.0, ""),
]


def extract_name(filename: str) -> str | None:
    """Extract a human-readable name from a photo filename."""
    ...
    clean = [p for p in parts if re.match(r"^[a-zA-Z]{2,}$", p)]
    if clean:
...
def load_photos(photo_dir: Path) -> tuple[list[Image.Image], list[str | None]]:
    """Load all child photos as cell-sized thumbnails + extract names."""
    photo_paths = sorted(
    ...
    for pp in photo_paths:
        try:
            ...
            photos.append(img)
            names.append(extract_name(pp.name))
        except Exception:
            pass
...
def main() -> None:
    """Generate the final V1 video."""
    OUTPUT_DIR.mkdir(exist_ok=True)

    print("🎬 Building Final Video V1...")
    ...
    output_path = OUTPUT_DIR / "100_faces_FINAL_V2.mp4"
    ...
    if proc.returncode != 0:
        ...
        print(f"\n🎉 FINAL VIDEO V2: {output_path}")
        print(f"   Size: {file_size_mb:.1f} MB")
        print(f"   Duration: {TOTAL_DURATION:.1f}s ({TOTAL_DURATION / 60:.1f} min)")
        print(f"   Resolution: {VIDEO_SIZE[0]}×{VIDEO_SIZE[1]}")
        print(f"   Audio: Suno vocals + subtle beat mix")
